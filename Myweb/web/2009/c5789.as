function fadedout()
{
    if (play_mode == "play")
    {
        mov_set_buttons["mov_set_button" + home_pos].colorTo(0, 5.000000E-001);
        ++home_pos;
        if (xml_file.firstChild.childNodes[home_pos].attributes.path == undefined)
        {
            home_pos = 0;
        } // end if
        mov_set_buttons["mov_set_button" + home_pos].colorTo(_global.color_on, 5.000000E-001);
        load_bk(xml_file.firstChild.childNodes[home_pos].attributes.path, mov_holder_fake);
    } // end if
} // End of the function
function load_bk(imagem, clip)
{
    var _loc2 = new MovieClipLoader();
    var _loc1 = new Object();
    _loc1.onLoadStart = function (clip)
    {
        mov_imgloader._alpha = 100;
        mov_imgloader._width = 0;
    };
    _loc1.onLoadProgress = function (m_clip, bytesLoaded, bytesTotal)
    {
        mov_imgloader._width = bytesLoaded / bytesTotal * 784;
    };
    _loc1.onLoadComplete = function ()
    {
        mov_holder_fake._visible = false;
        mov_imgloader.alphaTo(0, 3.000000E-001, "linear");
        mov_holder_home.alphaTo(0, 3.000000E-001, "linear", 0, function ()
        {
            bit_loader.loadClip(xml_file.firstChild.childNodes[home_pos].attributes.path, mov_holder_home);
        });
    };
...
home_pos = 0;
play_mode = "play";
mov_imgloader._alpha = 0;
mov_holder_fake._visible = false;
this.mov_set_buttons.mov_set_button._visible = false;
mov_set_buttons.mov_play_pause.colorTo(_global.color_on, 0);
mov_imgloader.colorTo(_global.color_on, 0);
var bit_loader = new MovieClipLoader();
var bit_listener = new Object();
bit_listener.onLoadStart = function (mov_holder_home)
{
};
bit_listener.onLoadProgress = function (mov_holder_home, bytesLoaded, bytesTotal)
{
};
bit_listener.onLoadInit = function (mov_holder_home)
{
    mov_holder_home.alphaTo(100, 3.000000E-001, "linear");
    if (play_mode == "play")
    {
        mov_holder_home.slideTo(3, 3, 1.000000E-001, "linear", xml_file.firstChild.childNodes[home_pos].attributes.display_time, function ()
        {
            fadedout();
        });
    } // end if
...
{
    load_bk(xml_file.firstChild.childNodes[home_pos].attributes.path, mov_holder_fake);
    for (i = 0; i < xml_file.firstChild.childNodes.length; i++)
    {
        mov_set_buttons.mov_set_button.duplicateMovieClip("mov_set_button" + i, i);
        mov_set_buttons["mov_set_button" + i]._x = i * 13;
        ...
        {
            show_tooltip(true, this.txt_cat, this);
        };
        mov_set_buttons["mov_set_button" + i].onRelease = function ()
        {
            ...
            home_pos = Number(this.pos);
            load_bk(xml_file.firstChild.childNodes[home_pos].attributes.path, mov_holder_fake);
        };
    } // end of for
    mov_set_buttons["mov_set_button" + home_pos].colorTo(_global.color_on, 0);
    mov_set_buttons.mov_play_pause._x = mov_set_buttons["mov_set_button" + (i - 1)]._x + 20;
    mov_set_buttons._x = 790 - mov_set_buttons._width - 5;
};
xml_file.load("intro.xml");
mov_set_buttons.mov_play_pause.onRelease = function ()
{
    mov_holder_home.stopTween();
    if (play_mode == "play")
    {
        mov_set_buttons.mov_play_pause.gotoAndStop(2);
        play_mode = "pause";
    }
    else
    {
        mov_set_buttons.mov_play_pause.gotoAndStop(1);
        play_mode = "play";
        mov_set_buttons["mov_set_button" + home_pos].colorTo(0, 5.000000E-001);
        ++home_pos;
        if (xml_file.firstChild.childNodes[home_pos].attributes.path == undefined)
...
receiving.stopMessage = function ()
{
    mov_holder_home.stopTween();
};
receiving.connect("stopConnection");
stop ();
