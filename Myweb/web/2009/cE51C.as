...
function load_image(imagem, clip)
{
    ...
    {
        mov_news.mov_imgloader._width = mov_news.mov_imgloader._width + Math.round((Math.round(bytesLoaded / bytesTotal * 350) - mov_news.mov_imgloader._width) / 8);
    ...
    {
        mov_news.mov_imgloader._width = 350;
...
function load_item(new_pos)
{
    mov_news.body_txt._x = 100;
    load_image(news_xml.firstChild.childNodes[new_pos].attributes.image, mov_news.mov_thumb);
    mov_news.txt_title.text = news_xml.firstChild.childNodes[new_pos].attributes.title.toUpperCase();
    mov_news.body_txt.htmlText = news_xml.firstChild.childNodes[new_pos].firstChild.nodeValue;
    mov_news.txt_title.alphaTo(100, 4.000000E-001);
    mov_news.body_txt.slideTo(370, y, 5.000000E-001);
} // End of the function
...
but_prev.mov_line_r.colorTo(_global.color_on, 0);
...
news_xml.onLoad = function ()
{
    ...
    if (news_xml.firstChild.childNodes.length > 0)
    {
        but_next.enabled = true;
        but_next.mov_line_r.colorTo(_global.color_on, 0);
    }
    else
    {
        but_next.enabled = false;
    } // end else if
    but_next.mov_line_r.colorTo(16777215, 0);
    for (var _loc3 = 0; _loc3 < _loc4.length; ++_loc3)
    {
        ...
        mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + _loc3].onRelease = function ()
        {
            if (this.pos != insider_pos)
            {
                mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + insider_pos].colorTo(6710886, 5.000000E-001, "linear");
                insider_pos = this.pos;
                if (insider_pos == 0)
                {
                    but_prev.enabled = false;
                ...
                if (insider_pos == news_xml.firstChild.childNodes.length - 1)
                {
                    but_next.enabled = false;
                ...
                mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + insider_pos].colorTo(_global.color_on, 5.000000E-001, "linear");
                mov_news.mov_left.colorTo(6710886, 3.000000E-001, "linear");
                mov_news.mov_left.enabled = false;
                cont = this.pos;
                mov_news.body_txt.slideTo(640, y, 3.000000E-001, "linear");
