function align_services()
{
    ...
    mov_set_buttons._x = -mov_news._parent._parent._x - mov_news._parent._x + _global.menu_xmargin;
    mov_set_buttons._y = -mov_news._parent._parent._y - mov_news._parent._y + _global.menu_ymargin;
} // End of the function
function load_image(imagem, clip)
{
    ...
    {
        mov_news.mov_imgloader._width = mov_news.mov_imgloader._width + Math.round((Math.round(bytesLoaded / bytesTotal * 320) - mov_news.mov_imgloader._width) / 8);
    ...
    {
        mov_news.mov_imgloader._width = 320;
...
function load_item(new_set, new_pos)
{
    insider_pos = new_pos;
    cont = new_pos + 1;
    this.mov_news.txt_view.text = "Viewing " + cont + " of " + news_xml.firstChild.childNodes[new_set].childNodes.length;
    if (news_xml.firstChild.childNodes[new_set].childNodes.length == 1)
    {
        mov_news.mov_right.colorTo(6710886, 3.000000E-001, "linear");
        mov_news.mov_right.enabled = false;
    } // end if
    mov_news.txt_date._y = 18;
    mov_news.txt_body._x = 75;
    ...
    mov_news.txt_date.slideTo(x, 38, 5.000000E-001);
    mov_news.txt_body.slideTo(342, y, 5.000000E-001);
    ...
    mov_news.txt_title.text = news_xml.firstChild.childNodes[new_set].childNodes[new_pos].attributes.title;
    mov_news.txt_date.text = news_xml.firstChild.childNodes[new_set].childNodes[new_pos].attributes.date.toUpperCase();
    mov_news.txt_body.htmlText = news_xml.firstChild.childNodes[new_set].childNodes[new_pos].firstChild.nodeValue;
} // End of the function
...
mov_news.txt_date.colorTo(_global.color_on, 0);
    ...
    for (var _loc3 = 0; _loc3 < _loc4.length; ++_loc3)
    {
        ...
        mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + _loc3].onRelease = function ()
        {
            if (this.pos != outsider_pos)
            {
                ...
                if (news_xml.firstChild.childNodes[outsider_pos].childNodes.length > 1)
                {
...
but_next.onRelease = function ()
{
    mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + outsider_pos].colorTo(6710886, 5.000000E-001, "linear");
    ++outsider_pos;
    mov_news._parent._parent._parent.mov_set_buttons["mov_set_button" + outsider_pos].colorTo(_global.color_on, 5.000000E-001, "linear");
    mov_news.txt_date.slideTo(x, 81, 2.000000E-001, "linear");
    mov_news.txt_body.slideTo(590, y, 2.000000E-001, "linear");
    mov_news.txt_title.alphaTo(0, 3.000000E-001, "linear");
    mov_news.mov_thumb.alphaTo(0, 3.000000E-001, "linear");
    mov_news.alphaTo(100, 0, "linear", 4.000000E-001, function ()
    {
        load_item(outsider_pos, 0);
    });
    if (outsider_pos == news_xml.firstChild.childNodes.length - 1)
    {
        this.enabled = false;
...
{
    align_services();
    delete mov_news.mov_entries.onEnterFrame;
