function make_scroll()
{
    ...
    {
        var _loc1 = (mov_dragger._y - initContentPos) / (scroll_height - mov_dragger._height);
        new_text_pos = Math.round(initContentPos - _loc1 * scroll_area);
    } // End of the function
    var scroll_height = mov_mask._height;
    var scroll_area = mov_text._height - mov_mask._height;
    var initContentPos = mov_text._y;
    var left = mov_bar._x;
    var top = mov_bar._y;
    var right = mov_bar._x;
    var bottom = mov_bar._y + mov_bar._height - mov_dragger._height;
    mov_dragger._y = top;
    new_text_pos = "";
    if (scroll_area < 0)
    {
        mov_dragger._visible = false;
        mov_bar._visible = false;
        return;
    }
    else
    {
        mov_dragger._visible = true;
        mov_bar._visible = true;
    } // end else if
    mov_dragger.onRollOver = function ()
    {
        this.mov_h_back.alphaTo(50, 5.000000E-001);
    };
    mov_dragger.onRollOut = function ()
    {
        this.mov_h_back.alphaTo(25, 5.000000E-001);
    };
    mov_dragger.onPress = function ()
    {
        this.mov_h_back.alphaTo(50, 5.000000E-001);
    ...
    _loc2.onMouseWheel = function (wheelNum)
    {
        if (mov_dragger._y >= top && mov_dragger._y <= bottom)
        {
            mov_dragger._y = mov_dragger._y - wheelNum * 5;
            if (mov_dragger._y < top)
            {
                mov_dragger._y = top;
            } // end if
            if (mov_dragger._y > bottom)
            {
                mov_dragger._y = bottom;
            } // end if
            updateContentPos();
        } // end if
    };
...
{
    mov_text._y = Math.round(mov_text._y - (mov_text._y - new_text_pos) / 4);
};
...
xml_file.onLoad = function ()
{
    ...
    mov_text.text_field.htmlText = xml_file.firstChild.firstChild.firstChild.nodeValue;
    mov_text.text_field.autoSize = "left";
    mov_dragger.alphaTo(100, 6.000000E-001, "linear", 1);
    mov_bar.alphaTo(20, 6.000000E-001, "linear", 1);
    mov_text.alphaTo(100, 4.000000E-001, "linear", 8.000000E-001);
    title_txt.alphaTo(100, 4.000000E-001, "linear", 8.000000E-001);
    make_scroll();
};
xml_file.load("about_us.xml");
